...
This is the minimum requirement to run the pipeline. However, if you have beta, odds ratio, Z-score or effect_allele_frequency, you have also give them standard headers to make sure they can be recognise correctly by the pipeline to be harmonised. Please refer to [GWAS Catalog webiste](https://www.ebi.ac.uk/gwas/docs/summary-statistics-format) for more inforation about standard headers.

For missing value, the input TSV file is processed using pandas. This means that missing values in other columns (outside of the required fields) are handled according to pandas' default settings and will not cause errors during processing. Ensure that required columns do not have missing values, while non-required fields with pandas-recognised missing value markers (e.g., NA, NaN, None) will be processed without issue.
...
#### Prepareing the sumstats data:
The process for preparing your input data depends on the number of summary statistics (sumstats) files you need to harmonize and the level of modifications required.
